    ...
    <div class="rtl">
        <mu-card class="mainContainer">
            <mu-container class="profile">
                <mu-row>
                    <mu-col span="12" sm="6">
                        <mu-card-header v-if="$auth.user().username != $route.params.username" class="profileHeader"
                            :title="user.first_name + ' ' + user.last_name" :sub-title="user.username">
                        ...
                        <mu-card-header v-if="$auth.user().username == $route.params.username" class="profileHeader"
                            :title="$auth.user().first_name + ' ' + $auth.user().last_name" :sub-title="$auth.user().username">
                            <mu-avatar slot="avatar" size="100">
                                <img :src="($auth.user().profile !== null) ? $auth.user().profile : '/img/profile.jpg'">
                            </mu-avatar>
                        </mu-card-header>
                    ...
                    <mu-col span="12" sm="6">
                        ...
                        <mu-card-actions v-if="user.username != undefined  && $route.params.username == $auth.user().username">
                            <mu-button @click="openFileUpload()">ویرایش پروفایل</mu-button>
                        </mu-card-actions>
                        <mu-card-actions v-if="user.username != undefined && $route.params.username != $auth.user().username">
                            <mu-button v-if="user.is_followed" flat @click="unfollowUser(user.username)" class="border">دنبال
                                نکن</mu-button>
        ...
        <mu-dialog title="ویرایش پروفایل" width="700" :open.sync="openUserpicDialog" class="rtl">
                ...
                <mu-row gutter>
                    <mu-col span="12" sm="12" md="12" lg="6" xl="6">
                        ...
                        <mu-form-item label="نوع صفحه">
                            <mu-checkbox v-model="editProfile.profileStatus" value="lock" uncheck-icon="public"
                                checked-icon="lock" :label="(editProfile.profileStatus) ? 'خصوصی' : 'عمومی'"></mu-checkbox>
                        </mu-form-item>
                    </mu-col>
                    <mu-col span="12" sm="12" md="12" lg="6" xl="6">
                        ...
                        <mu-form-item label="تکرار رمزعبور" prop="password_confirm" :rules="password_confirmRules">
                            <mu-text-field type="password" v-model="editProfile.password_confirm" prop="password_confirm"></mu-text-field>
                        </mu-form-item>
            ...
            <mu-button slot="actions" flat color="primary" @click="save">ثبت
            </mu-button>
            ...
            <div style="padding: 24px;" class="rtl">
                <mu-container>
                    ...
                    <div class="detailsOrg">
                        ...
                        <mu-row>
                            <mu-button color="error" v-if="selectedPost.is_liked" @click="unLikePost(selectedPost.id,selectedPost.index)">
                                <mu-icon right value="favorite"></mu-icon>پسندیدم
                            </mu-button>
                            <mu-button v-if="!selectedPost.is_liked" @click="likePost(selectedPost.id,selectedPost.index)">
                ...
                this.$refs.form.validate().then(result => {
                    if (result) {
                        ...
                        if (_this.editProfile.profile_photo != null)
                            formData.append("profile_photo", _this.editProfile.profile_photo, _this.editProfile
                                .profile_photo.name);
                        ...
                        if (_this.editProfile.profile_status)
                            ...
                            formData.append("profile_status", "PUBLIC");
